...
from models.fish_speech.conversation import (
...
from models.fish_speech.models.text2semantic.llama import BaseModelArgs
from models.fish_speech.text import clean_text, split_text
from models.fish_speech.tokenizer import IM_END_TOKEN, FishTokenizer
...
from models.fish_speech.models.text2semantic.llama import (
    BaseTransformer,
    DualARTransformer,
    NaiveTransformer,
)
...
def decode_one_token_ar(
    ...
    **sampling_kwargs,
    ...
    # Isolate tensors for CUDA graph steps
    x = model.forward_generate(x, input_pos)
    ...
    # Get first token with explicit detach and clone to isolate from CUDA graph
    token_result = sample(
        x.logits,
        previous_tokens=(
    ...
    # Create a completely new tensor to break CUDA graph dependencies
    first_token = token_result.detach().clone()
    codebooks = [first_token]

    # Get hidden states with proper isolation
    hidden_states = x.hidden_states.detach().clone()
    ...
    # Isolate intermediate computations
    a = codebooks[0].detach().clone() - model.tokenizer.semantic_begin_id
    a[a < 0] = 0
    hidden_states = model.fast_embeddings(a)
    codebooks.append(a.detach().clone())

    # Process remaining codebooks with proper isolation
    for codebook_idx in range(1, model.config.num_codebooks):
        ...
        # Create a completely new tensor to break CUDA graph dependencies
        a = token_result.detach().clone()
        
        hidden_states = model.fast_embeddings(a)
        codebooks.append(a)

    # Use fresh tensors for the final result
    isolated_codebooks = [t.detach().clone() for t in codebooks]
    codebooks_stacked = torch.stack(isolated_codebooks, dim=0)
    
    return codebooks_stacked
...
def decode_n_tokens(
    ...
    **sampling_kwargs,
    ...
    for i in tqdm(range(num_new_tokens)):
        ...
        # Remove the problematic methodization call
        # and use a simple context manager instead
        with torch._C.DisableTorchFunction():  # Alternative approach to disable optimizations
            with (
                ...
                next_token = decode_one_token(
                    model=model,
                    x=cur_token.detach().clone(),  # Explicitly clone inputs
                    input_pos=input_pos.clone(),   # Clone position tensor too
                    previous_tokens=window.clone() if window is not None else None,  # Clone window
                    semantic_ids=semantic_ids,
                    **sampling_kwargs,
                )

        # Ensure next_token is detached from computation graph

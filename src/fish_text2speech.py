...
class FishTextToSpeech:
    ...
    def load_model(self):
        ...
        state_dict = torch.load(
            self.checkpoint_path, map_location=self.device, mmap=True, weights_only=True
        )
        ...
        self.decoder, self.decode_one_token = init_encoder(
    ...
    def _warm_up_models(self):
        if self.decoder is None or self.encoder is None:
            raise RuntimeError(
                "Models not initialized or reference voice features not available for warm-up."
            )
        ...
        prompt_text = open(
            "/home/ubuntu/borhan/whole_pipeline/vexu/AI_TTS/src/test/ref/reference_text.txt",
            "r",
        ).readlines()[0]
        returned_data_tensor = inference_text2semantic(
            ...
            prompt_tokens=(
                "/home/ubuntu/borhan/whole_pipeline/vexu/AI_TTS/src/test/ref/reference_codec.npy",
            ),
            num_samples=1,
            max_new_tokens=1024,
            top_p=0.9,
        ...
        if isinstance(returned_data_tensor, torch.Tensor):
            ...
            semantic_tokens_save_path = (
                warmup_output_dir / "codes_0.npy"
            )  # Standard naming
        ...
        if (
            not generated_semantic_token_paths
            or not generated_semantic_token_paths[0].exists()
        ):
            return
        ...
        print(
            f"VQGAN warm-up pass completed. Warm-up audio saved in {warmup_output_dir}.",
            "magenta",
        )
    ...
    def voice_cloning(
        ...
        assert os.path.isfile(
            input_path
        ), f"input_path parameter should be a directory but {input_path} is file"
        ...
        logger.info(
            f"Loaded audio with {audios.shape[2] / self.encoder.sample_rate:.2f} seconds"
        )
    ...
    def synthesize_voice(
        self,
        text: str,
        output_codec_path_dir: str,
        reference_dir: str = "./test",
        output_wav_path: str = "./test",
        save_output: bool = True,
    ):
        """
        ...
        indices_lens = torch.tensor(
            [indices.shape[1]], device=self.device, dtype=torch.long
        )
        # Restore

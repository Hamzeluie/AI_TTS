...
def interrupt(stream):
        ...
        # for abort_event in stream.abort_events:
        #     abort_event.set()

        if stream.is_playing():
            stream.char_iter.stop()
            stream.player.resume()
            # stream.player.stop(immediate=True)
            stream.stream_running = False

        if stream.play_thread is not None:
            if stream.play_thread.is_alive():
                stream.play_thread.join()
            stream.play_thread = None
        ...
        try:
            ...
            async for message in websocket:
                ...
                if message == "END_OF_TEXT":
                    logger.info("Received END_OF_TEXT. Stopping TTS generator.")
                    client_word_queue.put(None)
                    break
                    
                logger.info(f"Received token: '{message}'")
                client_word_queue.put(message)
                
        except websockets.exceptions.ConnectionClosed:
            logger.info(f"Client {client_address} disconnected")
        except Exception as e:
            logger.exception(f"Error in receive_client_messages: {e}")
        finally:
            # Ensure the queue is signaled to stop
            client_word_queue.put(None)

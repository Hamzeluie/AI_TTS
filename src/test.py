...
class CoquiEngine(BaseEngine):
                ...
                if command == "update_reference":
                    ...
                    try:
                        ...
                        if full_sentences:
                            ...
                            for i, chunk in enumerate(chunks):
                                if stop_event.is_set():
                                    logging.info(
                                        "Stop event detected during chunk generation. Interrupting synthesis."
                                    )
                                ...
                                if i == 0:
                                    ...
                                    if load_balancing:
                                        if chunk_production_seconds < (
                                            ...
                                            if waiting_time > 0:
                                                print(
                                                    f"Waiting for {waiting_time} seconds"
                                                )
                        ...
                        if (
                            ...
                            and (full_generated_seconds - first_chunk_length_seconds)
                            > 0
                        ):
        ...
        with self._synthesize_lock:
            ...
            while "finished" not in status:
                ...
                if isinstance(result, bytes):
                    ...
                    logging.error(
                        f"Error in pipe, chunk bytes expected but string was sent: {result}"
                    )
    ...
    def set_stream_chunk_size(self, stream_chunk_size: int):
        ...
        """
        self.send_command(
            "set_stream_chunk_size", {"stream_chunk_size": stream_chunk_size}
        )
        status, result = self.parent_synthesize_pipe.recv()
        if status == "success":
            self.stream_chunk_size = stream_chunk_size

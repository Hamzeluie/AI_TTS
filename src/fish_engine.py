...
from fish_speech.fish_speech.models.dac.inference import load_model as load_vqgan_model
...
class FishEngine(BaseEngine):
    ...
    def create_worker_process(self):
        ...
        if not self.main_synthesize_ready_event.wait(timeout=60):
            ...
            raise RuntimeError(
                "Failed to initialize Fish TTS model within timeout period"
            )
    ...
    def _synthesize_worker(
        ...
        try:
            ...
            if not vqgan_checkpoint.exists():
                raise FileNotFoundError(
                    f"VQGAN checkpoint not found: {vqgan_checkpoint}"
                )
                ...
                try:
                    ...
                    if voice_path.exists():
                        if voice_path.suffix == ".npy":
                            ...
                            logging.info(
                                f"Using voice features from: {cloned_voice_features_path}"
                            )
                        elif voice_path.suffix == ".wav":
                            # Convert WAV to features
                            output_dir = Path(output_dir)
                            cloned_voice_features_path = (
                                output_dir / f"{voice_path.stem}_features.npy"
                            )
                            if not cloned_voice_features_path.exists():
                                logging.info(f"Extracting features from {voice_path}")
                                vqgan_infer_main(
                                    input_path=voice_path,
                                    output_path=cloned_voice_features_path.with_suffix(
                                        ".wav"
                                    ),
                                ...
                                logging.info(
                                    f"Voice features extracted to: {cloned_voice_features_path}"
                                )
                    else:
                ...
                try:
                    ...
                    if command == "synthesize":
                        ...
                        try:
                            ...
                            prompt_tokens = (
                                [str(cloned_voice_features_path)]
                                if cloned_voice_features_path
                                else []
                            )
                            prompt_text = (
                                [reference_prompt_text]
                            ...
                            if stop_event.is_set():
                                logging.info(
                                    "Synthesis stopped during token generation"
                                )
                                conn.send(("finished", ""))
                                continue
                            ...
                            if isinstance(semantic_tokens, torch.Tensor):
                                ...
                                tokens_tensor = (
                                    torch.from_numpy(np.load(semantic_tokens))
                                    .to(device)
                                    .long()
                                )
                            ...
                            logging.debug(
                                f"Audio generated, duration: {audio_duration}s"
                            )

                            if stop_event.is_set():
                                logging.info(
                                    "Synthesis stopped during audio generation"
                                )
                                conn.send(("finished", ""))
                                continue
                            ...
                            audio_array = (
                                audio_data[0, 0].float().detach().cpu().numpy()
                            )
                            ...
                            try:
                                ...
                                audio_array = scipy.signal.resample(
                                    audio_array, int(len(audio_array) * 24000 / 44100)
                                )
                            except ImportError:
                                logging.warning(
                                    "scipy not available for resampling, sending at original sample rate"
                                )
                            ...
                            for i in range(0, len(audio_array), chunk_size):
                                if stop_event.is_set():
                                    break
                                chunk = audio_array[i : i + chunk_size].astype(
                                    np.float32
                                )
                        ...
                        try:
                            ...
                            if voice_path.suffix == ".npy":
                                ...
                                cloned_voice_features_path = (
                                    output_dir_path / f"{voice_path.stem}_features.npy"
                                )
                                logging.info(f"Extracting features from {voice_path}")
                                vqgan_infer_main(
                                    input_path=voice_path,
                                ...
                                raise ValueError(
                                    f"Unsupported voice file format: {voice_path.suffix}"
                                )
        ...
        with self._synthesize_lock:
            ...
            while True:
                try:
                    ...
                    if response is None:
                        logging.error(
                            "Synthesis failed: Pipe connection broken or closed"
                        )
                        return False
    ...
    def set_voice(self, voice: Union[str, Path, FishVoice]):
        ...
        try:
            ...
            if self.parent_synthesize_pipe.poll(10):  # 10 second timeout
                ...
                if response is None:
                    logging.error(
                        "Failed to update voice: Pipe connection broken or closed"
                    )
                    return
    ...
    def shutdown(self):
        ...
        try:
            ...
            if self.synthesize_process.is_alive():
                logging.warning(
                    "Worker process did not exit gracefully, terminating..."
                )
